...
fn test_gitignore_negation_patterns() -> Result<(), Box<dyn Error>> {
    ...
    assert!(result.is_ignore(), "regular.log should be ignored by *.log pattern");
    ...
    assert!(!result.is_ignore(), "important.log should NOT be ignored due to negation pattern");
